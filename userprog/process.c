    ...
    if(strlen(argv) != 0)
    {
      ...
      for(i=0; i<argc; i++)
      {
        *esp -= strlen((const char*)token[argc-1-i])+1;
        ...
        memcpy(*esp, (void *)token[argc-1-i], strlen((const char*)token[argc-1-i])+1); //add args to stack
      }
...
setup_stack (void **esp) 
{
  ...
  if (kpage != NULL) 
    {
      success = install_page (((uint8_t *) PHYS_BASE) - PGSIZE, kpage, true); //Comparing with page size
      if (success)
        *esp = PHYS_BASE;
      else
        palloc_free_page (kpage);
